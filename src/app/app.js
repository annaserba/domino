...
app.controller("DominoController", ($scope) => {
  ...
  let mainDominoGroup = new fabric.Group([
    mainDominoGroupPart1,
    mainDominoGroupPart2
  ]);
  ...
  fabric.util.addListener(currentDomino.lowerCanvasEl, "mousedown", (options) => {
    if (dominos.length === 0 && options.layerX > mainDominoGroup.left &&
      options.layerY > mainDominoGroup.top &&
      options.layerX < mainDominoGroup.left + mainDominoGroup.width &&
      options.layerY < mainDominoGroup.top + mainDominoGroup.height) {
  ...
  fabric.util.addListener(newDomino.lowerCanvasEl, "mousedown", (options) => {
    for (let index = 0; index < dominos.length; index++) {
      if (options.layerX > dominos[index].domino.left &&
        options.layerY > dominos[index].domino.top &&
        options.layerX < dominos[index].domino.left + dominos[index].domino.width &&
        options.layerY < dominos[index].domino.top + dominos[index].domino.height) {
        $scope.changeDomino({
  ...
  $scope.changeDomino = (data) => {
    if (data.position == "Top") {
      ...
      mainDominoGroup = new fabric.Group([
        mainDominoGroupPart1,
        mainDominoGroupPart2
      ]);
      currentDomino.add(mainDominoGroup);
